...
if __name__=='__main__':
    ...
    for num_Seq in range(total_num_Seq):
        ...
        filename_list = sorted(glob.glob(Data_Dir+ Seperate + Seq + "/*.tfrecord"))  

        for scene_num, filename in enumerate(filename_list):
            print('Seq: %s, Scene: %d' %(str(num_Seq).zfill(4), scene_num) )

            # make directory
            save_path = Parsing_Dir + Seperate + Seq + '/scene_' + str(scene_num).zfill(2) 
            ...
            # Read one frame
            for frame_num, frame in enumerate(tqdm(frames)):
